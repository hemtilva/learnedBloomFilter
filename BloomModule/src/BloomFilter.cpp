...
BloomFilter::BloomFilter(size_t elementCount, double fpr){    
    size_t bits = BloomFilter::computeTheoryBits(elementCount,fpr);
    size_t hashes = BloomFilter::optimalHashes(bits,elementCount);
...
size_t BloomFilter::computeTheoryBits(size_t elementCount, double fpr){
    const double ln2 = log(2.0);
    double bits = -(elementCount * log(fpr)) / (ln2 * ln2);
    return static_cast<size_t>(ceil(bits));
}

size_t BloomFilter::optimalHashes(size_t bits, size_t elements){
...
size_t BloomFilter::getMemoryCost() const{
